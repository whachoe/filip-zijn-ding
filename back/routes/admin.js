...
router.post('/questions', async (req, res) => {
  try {
    ...
    if (!categories || !indicators) {
      return res.status(400).json({ error: 'Categories and indicators required' });
    }
...
router.put('/questions/:id', async (req, res) => {
  try {
    ...
    const result = await db.query(
      'UPDATE question_sets SET categories = $1, indicators = $2 WHERE id = $3 RETURNING *',
      [JSON.stringify(categories), JSON.stringify(indicators), req.params.id]
    );
